...
def analisador(glicerina, vaso, velocidad, calibracion_pos = 1, calibracion_vel = 1, grafico_centro = False):
    ...
    plt.plot(dists_plot[:-1]*calibracion_pos, np.abs(vts_plot)*calibracion_vel,'.', label ='Datos')
    plt.plot(dists_plot[:-1]*calibracion_pos, ran(dists_plot[:-1],*popt)*calibracion_vel, label = 'Ajuste Rankie')
    plt.plot(dists_plot[:-1]*calibracion_pos, bur(dists_plot[:-1],*popt2)*calibracion_vel, label ='Ajuste Burguers')
    plt.plot(dists_plot[:-1]*calibracion_pos, np.abs(np.array(vrs_plot))*calibracion_vel,'--k', label ='vr')
    plt.legend()
    plt.grid(alpha = 0.7)
